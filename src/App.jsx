...
// ─── API KEY — reads from Vite environment variable ──────────────────────────
...
function AddThinkerSearch({ onAdd, T }) {
  ...
  const search = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", { method:"POST", headers:{"x-api-key":ANTHROPIC_KEY,"anthropic-version":"2023-06-01"}, body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:400, messages:[{ role:"user", content:`Find psychologists, philosophers, or thinkers matching: "${q}". Return JSON array ≤4: [{name,domain,years,description}]. JSON only.` }] }) });
...
export default function PsychologistBlender() {
  ...
  const ask = async () => {
    ...
    try {
      const res  = await fetch("https://api.anthropic.com/v1/messages",{ method:"POST", headers:{"x-api-key":ANTHROPIC_KEY,"anthropic-version":"2023-06-01"}, body:JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:1200, messages:[{role:"user",content:prompt}] }) });
